...
class Optimizer(object):
    def __init__(self, model,scheduler,load_epoch=0):
        self.load_epoch=load_epoch
        self.scheduler=scheduler
        self.device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.save_dir = os.path.join('experiment', 'optim')
        ...
        def _get_optimizer(model):
            ...
            class _Optimizer(optimizer_class):
                def __init__(self, model, scheduler_class, kwargs_scheduler):
                ...
                def load_state_dict(self, state_dict, epoch=None):
                    ...
                    if isinstance(self.scheduler, lrs.MultiStepLR):
                        if [500,750,900] != list(self.scheduler.milestones) or 0.5 != self.scheduler.gamma:
                            reschedule = True
                    ...
                    if reschedule:
                        ...
                        self.scheduler.milestones = Counter([500,750,900])
            ...
            return _Optimizer(model,scheduler_class, kwargs_scheduler)
    ...
    def load(self, epoch):
        if epoch > 0:
            ...
            self.load_state_dict(torch.load(self._save_path(epoch), map_location=self.device), epoch=epoch)
