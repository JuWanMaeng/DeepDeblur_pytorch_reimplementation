...
class Loss(torch.nn.modules.loss._Loss):
    def __init__(self, epoch=1, model=None, optimizer=None):
        """
            input:
        ...
        self.loss_f='1*L2+10e-4*ADV'
        self.metric_s='PSNR,SSIM'
        self.rgb_range = 255
        ...
        for weighted_loss in self.loss_f.split('+'):
            ...
            if l in ('ABS', 'L1'):
                ...
                func = getattr(m, 'Adversarial')(model, optimizer)
            # else:
            #     loss_type = l
        ...
        # Metrics
        self.do_measure = self.metric_s.lower() != 'none'
        ...
        if self.do_measure:
            for metric_type in self.metric_s.split(','):
                metric_type = metric_type.upper()
                if metric_type == 'PSNR':
                    metric_func = PSNR()
                elif metric_type == 'SSIM':
                    metric_func = SSIM(self.device_type)    # single precision
                else:
        ...
        print('Metrics: {}'.format(self.metric_s))

        if self.start_epoch != 1:
            self.load(self.start_epoch - 1)
        ...
        self.to(self.device_type, dtype=self.dtype)
    ...
    def get_loss_desc(self):
        ...
        if self.mode in ('val', 'test'):
            metric_desc,result = self.get_metric_desc()
    ...
    def load(self, epoch=None):
        ...
        if os.path.exists(self.save_name):
            state = torch.load(self.save_name, map_location=self.device)
...
class Loss_test(torch.nn.modules.loss._Loss):
    def __init__(self, epoch=1, model=None, optimizer=None):
        """
            input:
                args.loss       use '+' to sum over different loss functions
